...
public class BuildVars {
    ...
    // Telegram API credentials are injected at build time from the gitignored root
    // local.properties (CYBERGRAM_API_ID / CYBERGRAM_API_HASH) via BuildConfig. The
    // upstream sample fallback is only used for non-Cybergram variants; the Cybergram
    // developer build fails at configuration time when the credentials are missing.
