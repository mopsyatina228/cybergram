...
A 2026-09-17 follow-up on the physically unlocked Redmi closed **page swipe, long-press/menu and
edit/reorder/delete mode** at P; the selected-chip label, unread counters and tapping-between-tabs are
also PASS. **Horizontal overflow/scroll is not testable** — the account has only four folders and they
fit on screen. The non-Cybergram control PASSED at P (upstream rounded chips).
...
exercised **all 15 rows**: rows 8, 10, 12, 14, 18, 25, 27, 28, 29, 31 and 35 PASS; rows 13, 19, 26 and
30 are PARTIAL (26 = the reactor popup scrim was captured on the physical phone, but the particle
animation is not observable in a still); **0 defects** were found. `E-rich` stays `UNAVAILABLE`.
