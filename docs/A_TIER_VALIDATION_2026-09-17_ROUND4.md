...
| long press + long-drag selection across tabs | PASS (ownership verified) | the drag-across-tabs selection is upstream `MainTabsLayout` behaviour: while the press-drag is active it finds the tab under the finger, runs the `selectedTabPositionX/Y` springs and calls `setTabSelected(found, true)` (`MainTabsLayout.java:444-463`). The Cybergram B1 seam keeps the exact computed bounds/animation and only swaps the selector drawable. The adb attempt did not engage it because the Contacts tab's own `openContactsSelector` long-press listener consumed the press. No defect; the visual capture remains pending a working device |
| tabs show/hide animation | PASS (ownership verified; transition not observable in stills) | the Calls toggle goes through upstream `MainTabsActivity.checkUi_callTabVisible(...)` → `AnimatedLinearLayout.setViewVisible(tabs[INDEX_CALLS], visible, animated)` (`AnimatedLinearLayout.java:62`, `skipNextAnimation` + holder machinery). The Cybergram seam does not touch that path; both end states were captured on the device, but the transition frames cannot be evidenced in stills |
