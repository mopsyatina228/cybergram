...
1. Chat header glass capsules around identity and action controls. Ownership is in the existing ActionBar/ChatAvatarContainer glass pipeline. The safest implementation seam is to suppress `ActionBar.setupGlass(...)` and `ChatAvatarContainer.setGlassMode()` only for Cybergram at the ChatActivity call site, leaving upstream/Day behaviour untouched. This needs a normal source edit + build/runtime pass before landing.
   - **LANDED 2026-09-10 (build + real-device validated).** The audit showed `ChatAvatarContainer.setGlassMode()` owns no surface (text/layout metrics only) and that `setupGlass` also carries positioning side effects plus the `setBackground(null)` that made the header transparent. The shipped seam is therefore narrower than proposed: an opt-in `ActionBar.setCybergramFlatHeader(boolean)` (default false) that keeps every glass-mode layout side effect, skips the three `BlurredBackgroundDrawable` capsules, and restores the opaque `key_actionBarDefault` panel; `ChatAvatarContainer.setGlassMode()` is deliberately retained. See `docs/WORK_STATE.md`, "Stage D pass 3: Cybergram flat chat header".
