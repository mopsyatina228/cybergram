...
> **Status banner (2026-09-16, later session):** B0 was **already run** on the authenticated AVD on
> 2026-09-15 and **stopped on a confirmed defect** — the selected chip rendered as an empty chamfered
> plate because the Cybergram plate is opaque and drawn after the labels (`FilterTabsView.java:1531`).
> The defect is **FIXED** (production commit `3911690fe`, E tier; evidence
> `docs/B0_FIX_IMPLEMENTATION_2026-09-16.md`), so this section re-runs as the **post-fix** matrix on the
> authenticated surface, including the non-Cybergram control. Finding record:
> `docs/B0_FILTER_TABS_DEFECT_2026-09-15.md`.
