...
- B6 — ordinary service/date implementation derived from B5: `READY / AUTHORIZED FOR BOUNDED IMPLEMENTATION` but `NOT IMPLEMENTED`; narrow scope is the ordinary `ChatActionCell.backgroundPath` only, with rich/special states, reactions and non-Cybergram presentation excluded;
