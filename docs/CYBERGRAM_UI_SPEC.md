...
The first visual reference is the generated chat concept discussed at project start: a direct Android screenshot, Telegram-like structure, dark background, amber/yellow incoming messages, cyan/dark outgoing messages, angular panel geometry, thin technical lines and restrained scanline/HUD details.
...
The first palette is deliberately small so it can be revised centrally.
...
- Muted text: `#7C8A91`
- Incoming-bubble dark text: `#101216`
- Outgoing bubble: `#0A1A21`
- Outgoing selected bubble: `#10313C`
- Outgoing light text: `#D7FCFF`

The checked-in `TMessagesProj/src/main/assets/cybergram.attheme` is the first executable approximation of this palette. It is not yet the final geometric treatment.
...
The header becomes a dark technical panel with cyan foreground elements, a thin red or amber state line used sparingly, and optional small HUD decoration in unused space.

Incoming text messages use an amber/yellow panel with dark text. Outgoing messages use a dark cyan panel with bright cyan/light text. Selected/pressed states increase contrast without changing the semantic side colours.

Delivery checks and outgoing metadata use cyan. Incoming metadata should remain darker and readable against amber.
...
The composer becomes a dark panel with cyan icons and cursor. The send button is cyan. Recording/destructive states may use the red accent.

## Message geometry

Phase 1 changes colour only through the theme system.

Phase 2 replaces rounded Telegram message silhouettes with an angular Cybergram drawable. Desired properties:

- clipped/angled corners rather than uniform radii;
- subtle 1 px-ish cyan/dark border for outgoing messages;
- optional dark inner keyline for incoming amber messages;
...
`MessageDrawable.java` is the primary rendering surface to investigate first. `ChatMessageCell.java` must be tested for grouped messages, media and message metadata before changing bubble bounds.
...
Avoid adding decorative separators that reduce visible conversation density.
...
Load/test `cybergram.attheme` and tune the core chat/list/action-bar colours. No custom bubble geometry yet.
...
Restyle `ChatActivityEnterView` and the chat action bar while preserving all existing input modes.

### Stage E: dialog list

Apply the same design system to the main conversation list, search and row states.
...
The existing theme system already exposes separate keys for incoming/outgoing bubble colours, text, link colours, checks, timestamps, panel colours and many related states. Use those keys before introducing new global styling hooks.
...
Do not attempt a whole-app visual rewrite before the chat vertical slice is working.
